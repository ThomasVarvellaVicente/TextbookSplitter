...
function createChapterRow() {
    ...
    div.innerHTML = `
        ...
        <div class="col-span-2 text-right">
            <button class="text-slate-500 hover:text-red-400 transition" onclick="this.parentElement.parentElement.remove()">
...
processBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        for (const chap of chapters) {
            ...
            // MATH: (Book Page + Offset) - 1 for 0-index
            const startIdx = (chap.start + offset) - 1;
            const endIdx = (chap.end + offset) - 1;

            if (startIdx < 0 || endIdx >= pdfDoc.getPageCount()) {
                console.error(`Range error for ${chap.title}`);
                continue;
            }
        ...
        status.innerText = "Building ZIP file...";
        ...
        console.error(err);
        status.innerText = "Error. Check file size/format.";
